...
test("postimages.org - short prompt", async () => {
  ...
  const prompt = fs.readFileSync(
    "tests/extract_requirements_prompts/prompts/short_prompt.txt",
    "utf8",
  );
  const pageContent = fs.readFileSync(
    "tests/extract_requirements_prompts/sources/postimages.org.txt",
    "utf8",
  );
  const outputSchema = JSON.parse(
    fs.readFileSync(
      "tests/extract_requirements_prompts/prompts/output_schema_images.txt",
      "utf8",
    ),
  );
  ...
  const { res, duration } = await page.evaluate(
    async (prompt, pageContent, outputSchema) => {
      ...
      const res = await session.prompt(pageContent, {
...
test("postimages.org - medium prompt", async () => {
  const browser = await puppeteer.connect({
    browserURL: "http://127.0.0.1:9222",
  });
  const page = await browser.newPage();
  ...
  const prompt = fs.readFileSync(
    "tests/extract_requirements_prompts/prompts/medium_prompt.txt",
    "utf8",
  );
  const pageContent = fs.readFileSync(
    "tests/extract_requirements_prompts/sources/postimages.org.txt",
...
test("postimages.org - large prompt", async () => {
  const browser = await puppeteer.connect({
    browserURL: "http://127.0.0.1:9222",
  });
  const page = await browser.newPage();
  ...
  const prompt = fs.readFileSync(
    "tests/extract_requirements_prompts/prompts/large_prompt.txt",
    "utf8",
  );
  const pageContent = fs.readFileSync(
    "tests/extract_requirements_prompts/sources/postimages.org.txt",
...
test("postimages.org - large prompt v2", async () => {
  const browser = await puppeteer.connect({
    browserURL: "http://127.0.0.1:9222",
  });
  const page = await browser.newPage();
  ...
  const prompt = fs.readFileSync(
    "tests/extract_requirements_prompts/prompts/large_prompt_v2.txt",
    "utf8",
  );
  const pageContent = fs.readFileSync(
    "tests/extract_requirements_prompts/sources/postimages.org.txt",
